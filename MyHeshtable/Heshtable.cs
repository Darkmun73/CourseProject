...
namespace MyHeshtable
{
    ...
    public class Heshtable
    {
        ...
        public int Add(Recipe recipe) //возвращает 0 - добавил, -2 - не добавил, потому что уже существует такая запись, -1 - не добавил потому что таблица заполнена
        {
            ...
            for(; (i < _size) && (_records[currentAddress].status != Status.empty); i++)
            {
                if ((_records[currentAddress].status == Status.filled)
                       ...
                       && (_records[currentAddress].recipe._author == key2)) { return -2; }  //если ячейка заполнена и ее ключ такой же как у добавляемой записи
            ...
            _records[currentAddress].firstHesh = firstHesh;
            return 0;
        }
